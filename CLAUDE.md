...
What a fresh session needs in place before running any `tea-scrape` command (cassette tests work without most of this; live scrapes need all of it):

1. **Postgres** — `docker compose up -d` from `tea-scrapers/` if the `tea-postgres` container is stopped. Container exposes `localhost:5432` with creds `postgres:postgres` and database `tea` (see `tea-scrapers/docker-compose.yml`). Container is healthchecked via `pg_isready`.
2. **Venv** — `~/.venvs/tea-scrapers/` (kept out of the repo tree because iCloud sync flags in-tree `.venv/` as hidden and breaks the editable-install `.pth`). The `tea-scrape` CLI lives at `~/.venvs/tea-scrapers/bin/tea-scrape` — call directly or `source ~/.venvs/tea-scrapers/bin/activate` first.
3. **`.env`** — copy `tea-scrapers/.env.example` → `tea-scrapers/.env`, then:
   - **Uncomment and set** `DATABASE_URL=postgresql+psycopg://postgres:postgres@localhost:5432/tea`. The example ships this line commented out; the Settings default (`postgresql+psycopg://localhost/tea`) has no creds and the docker container will reject with `fe_sendauth: no password supplied`. (Filed as spec §12 step-7 follow-up #7 — fix the example at next-touch.)
   - `USER_AGENT` and `REDDIT_USER_AGENT` are pre-filled with contact-bearing values that pass the placeholder-rejection validator as-is. Edit if you want to identify a different operator; do NOT revert them to the `gary@…` class default — Settings will refuse to start (V1.1 hardening).
4. **Schema** — `cd tea-scrapers && alembic upgrade head` lands at `002 (head)` and materializes all 10 tables with pg_trgm + HNSW indexes.
5. **Smoke check** — `cd tea-scrapers && pytest -q` should print `242 passed` (cassette-driven, no live HTTP, ~30s).

**Live-scrape gotchas worth knowing before kicking off a long-running command:**
- `tee` in a backgrounded pipeline masks the real exit code (`tee` always exits 0). Use `> /tmp/log 2>&1` instead of `| tee /tmp/log` for background scrapes.
- Steepster runs at `rate_limit_rps: 0.1` (honors `Crawl-Delay: 10`). A small `--max-teas 3` debug run takes ~30 min wall clock because the company-walk + 504 retries dominate. Don't underestimate.
- `yunnan_sourcing_com` will 403 on live scrapes today — JA3 fingerprint block, see spec §12 step-7 follow-up #9 for the mitigation plan. Other 3 Shopify vendors + Steepster work fine.
