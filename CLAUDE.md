...
Step 1 (Scaffolding) is complete — the `tea-scrapers/` subdirectory is the agreed layout, the package installs cleanly, the `tea-scrape` CLI shell runs, `alembic upgrade head` materializes all 10 tables with trigram + HNSW indexes, and the ruff + pytest gates pass. Postgres runs in the `tea-postgres` Docker container on `localhost:5432` (start with `docker compose up -d` from `tea-scrapers/` if a fresh session finds it stopped). Next unblocked task is **ingest step 2 (Shared infra)** per `ROADMAP.md:29`: `HttpClient` with rate limit + retry (scrapers spec §4), `JsonlWriter` with partitioning (§5), structlog config, and run tracking (§8) — owned by scraper-engineer.
