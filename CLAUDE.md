...
Steps 1 (Scaffolding), 2 (Shared infra), 3 (Shopify scraper), 4 (Run remaining Shopify vendors), and 5 (Bronze loader) are complete — the `tea-scrapers/` package installs into a venv at `~/.venvs/tea-scrapers` (out of `~/Desktop` because iCloud sync flags in-tree `.venv/` files as hidden, breaking the editable-install `.pth`), the `tea-scrape ingest shopify --vendor <key>` and `--all` CLI runs end-to-end against all 4 configured Shopify vendors (paginates `/products.json`, writes Hive-partitioned JSONL, finalizes the `scrape_run` row), `tea-scrape load --since YYYY-MM-DD` streams those JSONL files into `raw_product_snapshot` with `payload_hash` dedup (per-vendor batched `INSERT ... ON CONFLICT DO NOTHING` against `uq_raw_source_external_hash`, `scrape_run(source='loader', mode='bronze')`, exit codes 0/1/2 per §7), and **124 tests pass** (40 + 13 unit, 60 + 7 integration, all cassette-driven via vcrpy with one cassette + one 10-record golden JSONL per vendor). The `yunnan_sourcing_com` cassette is hand-trimmed to pages 1+2 + a synthesized empty page-3 terminator (artifact-level trim only, §11 anti-pattern preserved). `alembic upgrade head` materializes all 10 tables with trigram + HNSW indexes. Postgres runs in the `tea-postgres` Docker container on `localhost:5432` (`docker compose up -d` from `tea-scrapers/` if a fresh session finds it stopped). Active Shopify vendors: `yunnan_sourcing_us`, `yunnan_sourcing_com`, `white2tea`, `crimson_lotus`. **Known operational issue (§12):** the placeholder User-Agent in `.env.example` + sustained IP traffic triggers Shopify edge bot mitigation (403s on `white2tea`/`crimson_lotus` page 1, on `yunnan_sourcing_com` page 8) — V1.1 fix is to set a real `USER_AGENT` env var and cron the scraper. **Bronze loader follow-ups (§12, non-blocking):** rename `LoadStats._vendor_bucket` → `bucket_for`, add `LoadStats.insert_errors` counter, centralize `reset_session_caches()` test helper, strengthen `test_since_filter_skips_older_partitions` with distinct per-partition fixtures, broader §4 idempotency rewrite. Next unblocked task is **ingest step 6 (canonical ID matcher + silver normalizer)** per `ROADMAP.md:37`: Shopify products → `product` / `vendor_product` / `product_snapshot` rows with trigram index from day 1 — owned by data-engineer.
