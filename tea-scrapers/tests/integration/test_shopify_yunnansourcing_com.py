"""End-to-end VCR-replayed scrape of ``yunnansourcing.com``.

Spec reference: ``specs/tea_scrapers_v1_spec.md`` §9.

The cassette in ``fixtures/cassettes/yunnan_sourcing_com_products.yaml`` was
...
re-record (annually or when Shopify's payload shape shifts), run::
    ...
    VCR_RECORD_MODE=once pytest \\
        tests/integration/test_shopify_yunnansourcing_com.py::test_cli_full_run_against_cassette

and verify ``git diff`` shows no leaked cookies / UA email before committing.

Note: this cassette was recorded against the full ~12-page live scrape
then hand-trimmed in YAML to keep pages 1–2 plus a synthesized empty
page-3 terminator. The trim is artifact-level only — the scraper still
requests page 3 at run time; the cassette simply answers it as empty.
To re-record, capture the full scrape and re-apply the same trim
convention.
"""
...
CASSETTE = "yunnan_sourcing_com_products.yaml"
SOURCE_KEY = "yunnan_sourcing_com"
GOLDEN_FIXTURE = (
...
def test_cli_full_run_against_cassette(vcr_cassette, tmp_path: Path):
    ...
    for r in records:
        ...
        assert r.ingest_meta.endpoint.startswith("https://yunnansourcing.com/")
