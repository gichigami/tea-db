"""End-to-end VCR-replayed scrape of ``white2tea.com``.

Spec reference: ``specs/tea_scrapers_v1_spec.md`` §9.

The cassette in ``fixtures/cassettes/white2tea_products.yaml`` was
...
re-record (annually or when Shopify's payload shape shifts), run::
    ...
    VCR_RECORD_MODE=once pytest \\
        tests/integration/test_shopify_white2tea.py::test_cli_full_run_against_cassette
...
CASSETTE = "white2tea_products.yaml"
SOURCE_KEY = "white2tea"
GOLDEN_FIXTURE = (
...
def test_cli_full_run_against_cassette(vcr_cassette, tmp_path: Path):
    ...
    for r in records:
        ...
        assert r.ingest_meta.endpoint.startswith("https://white2tea.com/")
