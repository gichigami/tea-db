"""End-to-end VCR-replayed scrape of ``yunnansourcing.us``.

Spec reference: ``specs/tea_scrapers_v1_spec.md`` §9.

The cassette in ``fixtures/cassettes/yunnan_sourcing_us_products.yaml`` was
...
re-record (annually or when Shopify's payload shape shifts), run::
    ...
    VCR_RECORD_MODE=once pytest \\
        tests/integration/test_shopify_yunnansourcing_us.py::test_cli_full_run_against_cassette
...
CASSETTE = "yunnan_sourcing_us_products.yaml"
...
def test_cli_full_run_against_cassette(vcr_cassette, tmp_path: Path):
    ...
    for r in records:
        ...
        assert r.ingest_meta.endpoint.startswith("https://yunnansourcing.us/")
